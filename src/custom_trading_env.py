...
class CustomTradingEnv(gym.Env):
    ...
    def __init__(self, df, initial_balance=100000, lookback_window=30, transaction_cost=0.000, transition_steps=8000, max_episode_length=8000, term_train=False, DEBUG=False):
        """
    ...
    def _execute_trade(self, trading_action, current_price):
        ...
        if trading_action > 0:  # Buy
            ...
            if amount_to_buy > 0:
                ...
                transaction_fee = self.transaction_cost
                ...
                if shares_bought > 0:
                    ...
                    self.balance -= (actual_amount + transaction_fee)
    ...
    def _calculate_reward(self, realized_profit, current_price, trading_action, terminate_action=None):
        ...
        # PnL reward component
        if unrealized_pnl > 0:
            pnl_reward = (unrealized_pnl / self.initial_balance)
        else:
            pnl_reward = -(abs(unrealized_pnl) / self.initial_balance)
        ...
        # Risk penalty for excessive drawdown
        if drawdown > 0.15:  # 15% maximum acceptable drawdown
            risk_penalty = -0.5 * (drawdown - 0.15)  # Progressive penalty
        else:
            risk_penalty = 0
        ...
        # Normalize and scale realized profit
        realized_profit_reward = np.clip((realized_profit / self.initial_balance) * 10, -1, 1)
        
        # Combine reward components with appropriate weights
        reward = (
            0.8 * realized_profit_reward +  # Prioritize realized profits
            0.2 * pnl_reward +  # Consider unrealized PnL
            # 0.2 * sharpe_ratio +  # Risk adjustment
